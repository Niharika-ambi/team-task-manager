...
export default function Projects() {
  ...
  const fetchProjects = async () => {
    try {
      const res = await axios.get('https://team-task-manager-production-1006.up.railway.app/projects', {
  ...
  const createProject = async () => {
    try {
      await axios.post('https://team-task-manager-production-1006.up.railway.app/projects',
        { name, description },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      setName('')
  ...
  const addMember = async (projectId) => {
    try {
      const res = await axios.get('https://team-task-manager-production-1006.up.railway.app/auth/user-by-email?email=' + memberEmail, {
        headers: { Authorization: `Bearer ${token}` }
      })
      await axios.post(`https://team-task-manager-production-1006.up.railway.app/projects/${projectId}/members`,
        { user_id: res.data.id },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      setMemberEmail('')
