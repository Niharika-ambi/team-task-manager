...
export default function Tasks() {
  ...
  const fetchMembers = async (projectId) => {
    try {
      const res = await axios.get(`https://team-task-manager-production-1006.up.railway.app/projects/${projectId}/members`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      setMembers(res.data)
    } catch (err) { console.log(err) }
  ...
  const createTask = async () => {
    try {
      await axios.post('https://team-task-manager-production-1006.up.railway.app/tasks', form, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const updateStatus = async (taskId, status) => {
    try {
      await axios.patch(`https://team-task-manager-production-1006.up.railway.app/tasks/${taskId}/status`,
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      fetchTasks()
